...
public class ItemAdapter extends BaseExpandableListAdapter {
	...
	public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View view,
		...
		rightRow.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!dbSL.existsItem(item.getId()) && !dbBL.existsItem(item.getId())) {
	...
	public int getChildrenCount(int groupPosition) {
		if (collections != null && list != null && list.get(groupPosition) != null && collections.get(list.get(groupPosition)) != null) {
	...
	public void filter(String charText) {
		...
		if (charText.length() == 0) {
			...
			for (Item item : originalList) {
				ArrayList<Brand> brands = originalCollections.get(item);
	...
	private void editItem(final int position) {
		...
		} else if (list.get(position).getType() == 0 && list.get(position).getEssential() == 0){
			radioButtonQuantity.setChecked(true);
		} else {
		...
		textQuantity = (TextView) alertView.findViewById(R.id.alert_dialog_item_quantity);
		textQuantity.setText(""+list.get(position).getQuantity());
				...
				.setNegativeButton(R.string.alert_dialog_cancel, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (radioButtonQuantity.isChecked()) {
							item.setType(0);
							item.setQuantity(Integer.parseInt(textQuantity.getText().toString()));
						} else {
							item.setType(1);
						}
						...
						if(db.checkTypeChanges(item)){
							DBBrand dbBrand = new DBBrand(context);
							
							dbBrand.resetQuantityAndNeed(item.getId());
						}
