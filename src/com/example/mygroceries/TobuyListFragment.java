...
public class TobuyListFragment extends Fragment {
	...
	private void createParentList() {
		DBShoppingList dbSL = new DBShoppingList(rootView.getContext());
		DBBuyedList dbBL = new DBBuyedList(rootView.getContext());
		
		if(dbSL.emptyShoppingList() && dbBL.emptyBuyedList()) {
			DBItem db = new DBItem(rootView.getContext());
			parentList = db.shoppingList();
		} else {
			parentList = dbSL.getAllItems();
		}
	}
