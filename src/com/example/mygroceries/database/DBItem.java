...
public class DBItem extends DBManager<Item> {
	...
	public boolean checkTypeChanges(Item item){
		SQLiteDatabase db = this.getReadableDatabase();

		String selectQuery = "SELECT 1 FROM " + TABLE_ITEM + " WHERE "
				+ ID + " = " + item.getId() + " AND " + ITEM_TYPE + " = " + item.getType();
	...
	public boolean changeQuantity(Item item, int op, int num) {
		SQLiteDatabase db = this.getWritableDatabase();
		...
		if (op == 0) {
			if (item.getQuantity() - num >= 0) {
				values.put(ITEM_QUANTITY, item.getQuantity() - num);
			} else {
				closeDB();
	...
	public ArrayList<Item> shoppingList(){
		...
		String selectQuery = "SELECT * FROM " + TABLE_ITEM + " I WHERE " +
				...
				+ " OR I." + ITEM_TYPE + " = 1) "
				+ "AND EXISTS(SELECT 1 FROM " + TABLE_BRAND + " B " 
				+ "WHERE B." + BRAND_ITEM_ID + " = I." + ID + ")";
	...
	public boolean existsOnShopping(long id) {
		SQLiteDatabase db = this.getReadableDatabase();

		String selectQuery = "SELECT 1 FROM " + TABLE_ITEM + " I, " + TABLE_BRAND + " B "
				+ "WHERE EXISTS(SELECT 1 FROM " + TABLE_SHOPPING_LIST + " "
				+ "WHERE B." + SHOPPING_LIST_ITEM_ID + " = " + id + ") "
				+ "AND EXISTS(SELECT 1 FROM " + TABLE_BUYED_LIST + " "
